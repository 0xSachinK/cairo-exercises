# Exercise: https://www.cairo-lang.org/docs/hello_cairo/puzzle.html#exercise
# Play with the values of the location coordinates and make sure the program fails
# if they represent illegal values.

# For example, try to change loc_tuple[0].row from 0 to 10. You should see that the
# assert in verify_valid_location fails. Or you can change this value to 1, which will
# make the first transition illegal (the empty tile cannot stay in the same place).
...
func main():
    ...
    local loc_tuple : (
        ...
        # Fails with:
        # An ASSERT_EQ instruction failed: 0 != 5040.
        #     assert row * (row - 1) * (row - 2) * (row - 3) = 0
        #     ^************************************************^
        ...
        # Fails with:
        # An ASSERT_EQ instruction failed: 1 != 4.
        #         assert row_diff * row_diff = 1
        #         ^****************************^
